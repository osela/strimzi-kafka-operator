...
import static io.strimzi.systemtest.Resources.getBridgeServiceResource;
...
public class HttpBridgeST extends MessagingBaseST {
    ...
    void testHttpBridgeSendSimpleMessage(Vertx vertx) throws Exception {
        ...
        client.post(80, httpBridgeHost, "/topics/" + TOPIC_NAME)
    ...
    void createClassResources() throws Exception {
        ...
        testClassResources.createIngress(Resources.KAFKA_CLIENTS, Environment.KAFKA_CLIENTS_DEFAULT_PORT, CONFIG.getMasterUrl(), NAMESPACE).done();
        testClassResources.deployKafkaClients(CLUSTER_NAME, NAMESPACE).done();
        // Deploy http bridge
        service = getBridgeServiceResource(Environment.HTTP_BRIDGE_DEFAULT_PORT, "http");
        testClassResources.createServiceResource(service, NAMESPACE);
        testClassResources.createIngress(Constants.STRIMZI_BRIDGE_DEPLOYMENT_NAME, Environment.HTTP_BRIDGE_DEFAULT_PORT, CONFIG.getMasterUrl(), NAMESPACE).done();
        testClassResources.httpBridge(getBootstrapServer(CLUSTER_NAME), Environment.HTTP_BRIDGE_DEFAULT_PORT, NAMESPACE).done();
        // Set bridge host
        String kubernetesAddress = ENVIRONMENT.getKubernetesDomain().equals(Environment.KUBERNETES_DOMAIN_DEFAULT) ?
                new URL(CONFIG.getMasterUrl()).getHost() + Environment.KUBERNETES_DOMAIN_DEFAULT : ENVIRONMENT.getKubernetesDomain();
